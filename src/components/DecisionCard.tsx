...
export default function DecisionCard({
  ...
  const mainDecision = toDisplayText(decision.mainDecision);
  const recommendedChoice = toDisplayText(decision.recommendedChoice);
  const why = toDisplayText(decision.why);
  const keyRisk = toDisplayText(decision.keyRisk);
  const details = toDisplayList(decision.details);
  const alternatives = toDisplayList(decision.alternatives);
  ...
  return (
    <div className="vp-card" style={{ marginBottom: 18, borderColor: 'rgba(18, 18, 18, 0.24)', boxShadow: '0 24px 70px rgba(0, 0, 0, 0.12)' }}>
      ...
      <h2 style={{ fontSize: 22, lineHeight: 1.35, fontWeight: 700, letterSpacing: '-0.02em', marginBottom: 14 }}>{mainDecision}</h2>
      ...
      <div style={{ padding: 14, borderRadius: 12, background: 'var(--color-primary-light)', border: '1px solid rgba(18, 18, 18, 0.12)', marginBottom: 12 }}>
        <div style={{ display: 'flex', gap: 8, alignItems: 'flex-start' }}>
          ...
          <div>
            ...
            <div style={{ whiteSpace: 'pre-wrap', fontSize: 15, color: 'var(--color-text)', lineHeight: 1.7 }}>{recommendedChoice || '等待 AI 生成推荐方案。'}</div>
          </div>
        </div>
      </div>

      <p style={{ fontSize: 13, color: 'var(--color-text-secondary)', lineHeight: 1.7, marginBottom: 10 }}>
        <strong style={{ color: 'var(--color-text)' }}>为什么：</strong>{why || '先让 AI 完成分析，用户只确认关键决策。'}
      </p>
      ...
      <div style={{ display: 'flex', gap: 8, alignItems: 'flex-start', padding: 10, borderRadius: 10, background: 'var(--color-warning-light)', marginBottom: 14 }}>
        ...
        <p style={{ fontSize: 13, color: 'var(--color-text-secondary)', lineHeight: 1.6 }}><strong style={{ color: 'var(--color-warning)' }}>最大风险：</strong>{keyRisk}</p>
      </div>
        ...
        <div style={{ marginTop: 16, paddingTop: 16, borderTop: '1px solid var(--color-border)' }}>
          ...
          {!!alternatives.length && (
            <p style={{ fontSize: 13, color: 'var(--color-text-secondary)', lineHeight: 1.7, marginBottom: 12 }}>
              <strong style={{ color: 'var(--color-text)' }}>替代方案：</strong>{alternatives.join(' / ')}
            </p>
          )}
          ...
          {explanation && (
            <div style={{ marginTop: 12, padding: 12, borderRadius: 8, background: 'var(--color-bg)', border: '1px solid var(--color-border)' }}>
              <p style={{ fontSize: 13, color: 'var(--color-text-secondary)', lineHeight: 1.7 }}>{toDisplayText(explanation)}</p>
            </div>
          )}
