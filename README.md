...
## 🚀 V4.5 Runtime Consistency & Source-of-Truth Patch

V4.5 is a stability patch that fixes critical runtime issues and aligns code with V4.4's API-required philosophy.

### What changed:
- **Event Persistence** — All graph runtime events now persist to `session.events`, visible in EventTimeline and conversation.
- **Agent Reply Fix** — `userVisibleReply` from action intents (continue/skip/assume/generate_handoff) now generates durable `agent_message` events instead of disappearing after the thinking bubble.
- **AI Call Visibility** — `ai_call_started/completed/failed` events persist to session and are visible in the UI. New `lastAIStatus` state field.
- **Handoff Cleanup** — `GENERATE_HANDOFF` command now calls `optimizeHandoffWithAI` (AI-only). `generateLocalHandoff` renamed to legacy/debug tool.
- **Version Unification** — All version labels updated to V4.5.
- **DecisionOutput Fix** — Removed `useMemo` side effect that caused duplicate decision log writes. Added `DecisionSpecBundle` builder.
- **OutputSource Clarified** — Legacy `local-rule`/`mock` sources preserved for history but marked as deprecated for new messages.

### V4.5 Tech
- `src/agent-v4/graphRuntime.ts` — `appendRuntimeEvent` / `appendAgentReply` helpers, lastAIStatus tracking
- `src/agent-v4/types.ts` — `lastAIStatus`, `lastAIError`, `lastAINodeId`, `lastAICalledAt` fields
- `src/agent-v4/tools/toolRegistry.ts` — `generateLocalHandoff` → `legacyGenerateLocalHandoff`
- `src/agent-v4/tools/handoffTools.ts` — Legacy annotations
- `src/lib/decisionSpecBuilder.ts` — New `DecisionSpecBundle` builder
- `src/pages/DecisionOutputPage.tsx` — useMemo side effect removed
- `src/pages/AgentWorkspacePageV4.tsx` — AI Status card, DecisionOutput button, V4.5 labels
- `src/pages/HistoryPage.tsx` — DecisionOutput button
- `src/types.ts` — OutputSource comment
- `src/hooks/useProductBrief.ts` — normalizeSource legacy handling
...
- 如何在纯前端 localStorage 架构下实现 Agent 状态机、事件持久化和知识检索？
...
- 如何设计 Agent 运行时的事件持久化机制（events → session.events → localStorage）？
